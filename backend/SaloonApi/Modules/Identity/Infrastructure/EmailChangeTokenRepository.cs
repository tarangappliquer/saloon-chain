...
// Same opaque/hashed/single-use pattern as PasswordResetTokenRepository -- see dbo.EmailChangeTokens.
internal sealed class EmailChangeTokenRepository(SqlConnectionFactory factory)
{
    public async Task<int> CreateAsync(int userId, string newEmail, byte[] tokenHash, DateTime expiresAt)
    {
        ...
        await db.ExecuteSpAsync("dbo.sp_Auth_CreateEmailChangeToken", p);
        return p.Get<int>("@Id");
    }
    ...
    {
        using var db = factory.Create();
        return await db.QuerySingleSpAsync<EmailChangeTokenRecord>("dbo.sp_Auth_GetEmailChangeToken", new { TokenHash = tokenHash });
    }

    public async Task ConfirmAsync(int id, int userId, string newEmail)
    {
        using var db = factory.Create();
        await db.ExecuteSpAsync("dbo.sp_Auth_ConfirmEmailChange", new { Id = id, UserId = userId, NewEmail = newEmail });
    }
}
