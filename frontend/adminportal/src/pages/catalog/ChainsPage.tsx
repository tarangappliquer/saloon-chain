...
export function ChainsPage() {
  ...
  async function toggleActive(chain: Chain) {
    setError(null);
    try {
      await api.put(`/api/admin/catalog/chains/${chain.id}`, { name: chain.name, isActive: !chain.isActive });
      await load();
    } catch (err) {
            ...
            {chains.map((c) => (
              <tr key={c.id} className="border-b border-gray-100 dark:border-gray-900">
                ...
                <td className="py-2 text-right">
                  <button type="button" onClick={() => toggleActive(c)} className="text-purple-600 hover:underline">
