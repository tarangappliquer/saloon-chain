...
export function bookingStreamUrl(locationId: number, date: string): string {
  return `${API_BASE}/api/booking/stream?${new URLSearchParams({ locationId: String(locationId), date })}`;
}
