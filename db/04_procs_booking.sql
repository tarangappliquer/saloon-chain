...
BEGIN
    ...
    SELECT l.OpenTime, l.CloseTime, l.WorkingDaysMask,
           CASE WHEN EXISTS (
               SELECT 1 FROM dbo.LocationHolidays h WHERE h.LocationId = l.Id AND h.HolidayDate = @WorkDate
    ...
    WHERE CAST(b.StartTime AS DATE) = @WorkDate
...
BEGIN
    ...
    INSERT INTO dbo.Bookings (LocationId, RoomId, TherapistId, CustomerId, StartTime, EndTime, Status, ExpiresAt)
    VALUES (@LocationId, @RoomId, @TherapistId, @CustomerId, @StartTime, @EndTime, 'Held', @ExpiresAt);

    SET @BookingId = SCOPE_IDENTITY();

    INSERT INTO dbo.BookingTreatments (BookingId, TreatmentId, SequenceOrder, SlotCount, Price)
    SELECT @BookingId, t.Id, ROW_NUMBER() OVER (ORDER BY t.Id), t.DurationSlots, COALESCE(lt.PriceOverride, t.Price)
    FROM dbo.Treatments t
...
-- Called every ~30s by a background job. Returns the (location,room,date) tuples that flipped
-- so the API can invalidate the availability cache and nudge SSE subscribers for just those groups.
...
BEGIN
    ...
    WHERE Status = 'Held' AND ExpiresAt <= SYSUTCDATETIME();
...
BEGIN
    ...
    WHERE b.CustomerId = @CustomerId AND b.Status IN ('Held','Confirmed')
